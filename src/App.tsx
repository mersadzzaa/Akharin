...
const App: React.FC = () => {
  ...
  const cleanupAndLeave = async () => {
    ...
    const currentState = data.data as GameState;
    ...
    if (myIdx !== -1) {
      ...
      if (remainingPlayers === 0) {
        ...
        await supabase.from("rooms").delete().eq("room_id", currentRoomId);
      } else {
        // Just update connection status or remove player
        currentState.players[myIdx] = null; // Physically remove from seat
        await supabase
          .from("rooms")
          .update({ data: currentState })
          .eq("room_id", currentRoomId);
      }
    }
  };

  // Hook for window close / refresh
  useEffect(() => {
    const handleUnload = () => {
      // Note: Async calls in unload are unreliable, but we try standard fetch/beacon if needed.
      // Supabase js might not finish.
      // However, for SPA navigation (back button handled by React), useEffect return works.
      cleanupAndLeave();
    };
    ...
    return () => {
      ...
      // Also run cleanup on component unmount (if user clicks "Back" in browser which unmounts App)
  ...
  useEffect(() => {
    ...
    const validateRoom = async () => {
      try {
        ...
        if (rData.phase === GamePhase.MATCH_END) {
          localStorage.removeItem("last_room_id");
          setLastRoomId("");
          return;
        }
  ...
  useEffect(() => {
    ...
    const fetchLatestState = async () => {
      ...
      if (data && data.data) {
        ...
        // If room deleted (null data), handle exit
        if (!data) {
          setInGame(false);
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          // Room deleted by host/last player
          setInGame(false);
          setError("اتاق بسته شد.");
          setRoomId("");
          localStorage.removeItem("last_room_id");
    ...
    // 3. Polling Fallback (Every 3 seconds) - GUARANTEES connection
    const pollInterval = setInterval(fetchLatestState, 3000);

    // 4. Simple Presence (Heartbeat)
    ...
    if (
      ...
      const interval = setInterval(() => {
        ...
        if (card.rank === Rank.Ace) {
          clearInterval(interval);
          // Logic: Card 1 is for Player 0, Card 2 is for Player 1, etc.
          // index 1 -> player 0. index 2 -> player 1.
          ...
          if (winner) {
            ...
            }, 3000); // Wait a bit so they see the Ace
          }
        }
      }, 1500);
  ...
  const joinRoom = async (specificRoomId?: string) => {
    ...
    try {
      await performTransaction((roomData) => {
        ...
        if (existingIdx !== -1) return roomData;
  ...
  const handleJoinPublicRoom = async () => {
    ...
    try {
      ...
      if (!data) {
        setLoading(false);
        alert("هنوز اتاقی ساخته نشده است.");
        return;
      }
  ...
  if (gameState.phase === GamePhase.MATCH_END) {
    ...
    return (
      <div className="fixed inset-0 z-[100] bg-gradient-to-b from-gray-900 to-black flex flex-col items-center justify-center p-4 text-white text-center overflow-y-auto">
        ...
        <button
          onClick={() => {
            updateRemoteState({ roomId: "DELETED" }); // Mark to trigger cleanup or just local refresh
            // In Supabase generic row deletion is not authed usually, so we just reset local
